...
```
按照我的这种写法，not between的含义是取除了在这段范围，例如我一个会员在3号，5号，
10号有购物记录，
not between4号和7号，那么会不取5号这条记录，只取3号和10号这两天购物记录，
但这改变不了这个人在4号到7号之间有购物的事实。
...
3.看日志要带着问题去看日志，出现了错误了，看ERROR，如果是运行没有错误，
但返回的结果不是自己想要的等等，要看状态，INFO,DEBUG,WARN等
...
![](26.jpg)
```
    git 只merge单个文件：git checkout --patch  <分支名> <路径名>
```
```
...
```
  1.SQL SERVER中 RIGHT()函数从指定字符串的右侧提取给定数量的字符。 例如，
  RIGHT('SQL Server'，6)返回：'Server'。

  2.SQL SERVER中 SQL CHARINDEX函数返回字符或者字符串在另一个字符串中的起始位置

  3.关于索引，联合索引查询要按照顺序用索引，用的越多，相当于缩小范围后再缩小范围，
  但是联合索引要按照顺序用，否则命中不了，联合索引一般用在多个字段关联作为查询条件，用之前
  先思考业务场景。索引碎片一般是发生在删除操作后，那条数据会以null值的形式存放在数据库中，
  形成索引碎片，一些针对小数据量的数据库可以用一些工具进行清理，一些针对大数据量存储的数据库
  可能不支持删除甚至修改操作。

  建立索引后依然可以插入数据，只不过索引会重新进行构建。
```
...
![](25.jpg)
```
  Preserve log会保留之前访问打出来的日志，Disable cache会不从浏览器缓存(memory cache)读取访问，访问速度
  会变慢，但内容是最新的，前端的人员调试这个选项会一直勾选的。
```